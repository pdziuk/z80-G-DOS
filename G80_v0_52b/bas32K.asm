...
STACKBAS EQU    WRKSPC+66H          ; Initial stack
...
INITMS:   LD      DE,INITAB       ; Initialise workspace
...
WARMST: LD      SP,STACKBAS     ; Temporary stack
...
INITAB: JP      WARMST          ; Warm start jump
        ...
        DB   56              ; Width for commas (5 columns)
...
OTHERKEY:
        LD(INKEYVALUE), A       ; Store keypress for INKEY Function
        RET
...
IFF:     CALL    EVAL            ; Evaluate expression
        LD      A,(HL)          ; Get token
        CP      ZGOTO           ; "GOTO" token?
...
INKEY:  LD A,(INKEYVALUE)       ; Get key pressed, null if no key pressed
        PUSH AF                 ; Save the key
        LD A, 00H               ; Reset to no key pressed for next time
        LD (INKEYVALUE),A
        POP AF                  ; Get orig key pressed
...
CLSBAS: 
        ...
        LD      HL,CLSSTR       ; Clear screen codes
        CALL    PRS             ; Output string
...
INVERSEBAS: 
        PUSH    HL              ; Save HL register
        LD      HL,INVSTR       ; Inverse text screen codes
        CALL    PRS             ; Output string
        POP     HL              ; Restore HL register
        XOR     A               ; Set to position 0
        LD      (CURPOS),A      ; Store it
...
FLASHBAS: 
        PUSH    HL              ; Save HL register
        LD      HL,FLASHSTR     ; Flash text screen codes
        CALL    PRS             ; Output string
        POP     HL              ; Restore HL register
        XOR     A               ; Set to position 0
        LD      (CURPOS),A      ; Store it
...
NORMALBAS:
        PUSH    HL              ; Save HL register
        LD      HL,NORMALSTR       ; Normal text screen codes
        CALL    PRS             ; Output string
        POP     HL              ; Restore HL register
        XOR     A               ; Set to position 0
        LD      (CURPOS),A      ; Store it
...
SCREEN: CALL    GETINT          ; set cursor position to column, row - first get column
        PUSH    AF              ; Save column
        CALL    CHKSYN          ; Make sure ',' follows
        DB   ','
        CALL    GETINT          ; Get integer row
        PUSH    AF              ; Save row
        
        PUSH    HL
        LD      HL,SCREEN1STR   ; "ESC["
        CALL    PRS             ; Output string
        POP     HL
...
COLOR:  CALL    GETINT          ; set foreground and background color
        PUSH    AF              ; Save foreground color
        CALL    CHKSYN          ; Make sure ',' follows
        DB   ','
        CALL    GETINT          ; Get integer background color
        PUSH    AF              ; Save background
        ...
        POP     AF              ; Restore background
        ADD     A,40              ; add 40 decimal to get background range
        PUSH    HL
        LD      H,00
        LD      L,A
        CALL    PRNTHL
        ...
        POP     AF              ; Restore foreground
        ADD     A,30              ; add 40 decimal to get background range
        PUSH    HL
        LD      H,00
        LD      L,A
        CALL    PRNTHL
        
        LD      HL,SCREEN4STR   ; print out "m"
        CALL    PRS
...
CLSSTR: DB      ESC,"[2J",ESC,"[H",00H
INVSTR: DB      ESC,"[7m",00H
FLASHSTR:
        DB      ESC,"[5m",00H
NORMALSTR:
        DB      ESC,"[0m",00H
...
DEFN:   LD C,00H                ;CHECK IF END OF LINE
        CP C
        JR Z,DOS_ERASE          ;IF END OF WORD TRY TO ERASE
        LD (DE),A               ;STORE IN FILENAME BUFFER
        INC DE
        CALL GETCHR             ;GET THE CHAR FROM BUFFER
        JP DEFN                 ;GET NEXT CHARACTER
DOS_ERASE:
        LD A,00H
        LD (DE),A
        PUSH HL
...
MSBASIC_SAVE:                    ; This Saves the current program to USB Drive with the given name.
        push de
        push hl
        push af
        call msbasic_get_program_size
        ld a, h
        or l
        cp 0
...
msbasic_save_continue1:
        ...
        pop hl
        LD DE,filename_buffer   ;Pointer to filename buffer
DSFN:   LD C,00H                ;CHECK IF END OF LINE
        CP C
        JR Z,DOS_SAVE           ;IF END OF WORD TRY TO ERASE
        LD (DE),A               ;STORE IN FILENAME BUFFER
        INC DE
        CALL GETCHR             ;GET THE CHAR FROM BUFFER
        JP DSFN                 ;GET NEXT CHARACTER
DOS_SAVE:
        LD A,00H
        LD (DE),A
...
msbasic_get_program_size:
        ...
        ;ld hl, 0FDFFH
        LD   HL,(PROGND)       ;OP1 INTO HL
        ;LD  DE,(BASTXT)       ;OP2 INTO DE
        LD  DE, 8000H         ;OP2 INTO DE
        AND  A                 ;CLEARY CARRY
        SBC  HL,DE             ;OP1 - OP2
        ret
...
MSBASIC_LOAD:
        PUSH DE
        LD DE,filename_buffer   ;Pointer to filename buffer
DLFN:   LD C,00H                ;CHECK IF END OF LINE
        CP C
        JR Z,DOS_LOAD           ;IF END OF WORD TRY TO ERASE
        LD (DE),A               ;STORE IN FILENAME BUFFER
        INC DE
        CALL GETCHR             ;GET THE CHAR FROM BUFFER
        JP DLFN                 ;GET NEXT CHARACTER
DOS_LOAD:
        LD A,00H
        LD (DE),A
...
msbasic_load_loop1:
        ...
        call read_data_byte
        pop hl
        jp msbasic_load_loop1
msbasic_load_finished:
        ;ld (TXTUNF), hl
        call close_file
        pop hl
        pop de
