...
message:
	...
	push af					; We do this 5 times
	push af					; to allow spare stack space.
							; sp -> AF, AF, AF, AF, AF, ret-addr
	push bc					; sp -> BC, AF, AF, AF, AF, AF, ret-addr
	push de					; sp -> DE, BC, AF, AF, AF, AF, AF, ret-addr
	push hl					; sp -> HL, DE, BC, AF, AF, AF, AF, AF, ret-addr

	inc sp
	inc sp					; adjust the stack to overlook the stored afx3, BC, DE & HL
							; HL, sp -> DE, BC, AF, AF, AF, AF, AF, AF, ret-addr
	inc sp
	inc sp					; HL, DE, sp -> BC, AF, AF, AF, AF, AF, ret-addr

	inc sp
	inc sp					; HL, DE, BC, sp -> AF, AF, AF, AF, AF, ret-addr
	...
	inc sp
	inc sp					; HL, DE, BC, AF, AF, AF, AF, AF, sp -> ret-addr

	ex (sp), hl				; top of stack is now mangled, but hl is pointing to our message
							; HL, DE, BC, AF, AF, AF, AF, AF, sp -> HL
...
message_loop:
	...
	inc hl
	call PRINT_CHAR			; print a character (Mangles 4 items below top of stack)
	jr message_loop			; Loop until done
							; HL, DE, BC, AF, XX, XX, XX, XX, sp -> HL

message_complete:
	inc hl
	ex (sp), hl				; restore top of stack, after we have incremented it so it points to the subsequent instruction
							; HL, DE, BC, AF, XX, XX, XX, XX, sp -> new-ret-addr
	dec sp
	dec sp
	...
	dec sp					; adjust stack because of our pushed "af"
							; HL, DE, BC, sp -> AF, XX, XX, XX, XX, new-ret-addr
	dec sp
	dec sp					; adjust stack because of our pushed "BC"
							; HL, DE, sp -> BC, AF, XX, XX, XX, XX, new-ret-addr
	dec sp
	dec sp					; adjust stack because of our pushed "DE"
							; HL, sp -> DE, BC, AF, XX, XX, XX, XX, new-ret-addr
	dec sp
	dec sp					; adjust stack because of our pushed "HL"
							; sp -> HL, DE, BC, AF, XX, XX, XX, XX, new-ret-addr

	pop hl					; HL is restored
							; sp -> DE, BC, AF, XX, XX, XX, XX, new-ret-addr
	pop de					; DE is restored
							; sp -> BC, AF, XX, XX, XX, XX, new-ret-addr
	pop bc					; BC is restored
							; sp -> AF, XX, XX, XX, XX, new-ret-addr
	pop af					; we have restored af
							; sp -> XX, XX, XX, XX, new-ret-addr
